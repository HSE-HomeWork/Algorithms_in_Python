...
UNEVEN_AR: list[int] = [1, 1]
EMPTY: list[int] = []
NORMAL_AR: list[int] = [2, 3, 4]

HOMEWORK_AR: list[int] = [5, 7, 13, 2, 14]
...
def test_empty_array() -> None:
    ...
    assert max_even_sum_array(EMPTY) == 0


def test_uneven_arrays() -> None:
    """Поиск суммы в нечётном массиве."""
    answer = 2
    assert max_even_sum_array(UNEVEN_AR) == answer


def test_normal_arrays() -> None:
    """Поиск суммы в стандартном массиве."""
    answer = 6
    assert max_even_sum_array(NORMAL_AR) == answer
